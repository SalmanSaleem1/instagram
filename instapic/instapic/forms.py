...
from .models import User, Photo, PhotoTag, PhotoLikes, Followers
...
class AjaxSavePhoto(Ajax):
    def validate(self):
        ...
        tag_count = 0
        p = Photo(url=self.url, baseurl=self.baseurl, owner=self.user.username, email=self.user.email, likes=0, caption=self.caption,
                  main_colour=main_colour)
        p.save()
        if data["faces"] != []:
            for face in data["faces"]:
                PhotoTag(photoid=p.id, coords=face).save()
        ...
        for item in Photo.objects.filter(owner__in=followerslist).order_by('-date_uploaded')[
            ...
            out.append(
                {"PostID": item.id, "URL": item.url, "Caption": item.caption, "Owner": item.owner, "Likes": item.likes,
                 ...
                 "ProfilePic": profilepics[item.owner], "Liked": liked , "MainColour": item.main_colour, 'Email':item.email})
...
class AjaxFollow(Ajax):
    def validate(self):
        ...
        if not Followers.objects.filter(user=self.follower, follower=self.user.username).exists():
            ...
            # Followers(user=self.follower, follower=self.user.username).save()
            following = True
        else:
